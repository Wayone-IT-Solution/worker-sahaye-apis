...
export class PersonalAssistantController {
  static async createPersonalAssistant(
    ...
    try {
      ...
      if (!profileImageUrl) return res.status(403).json(new ApiError(403, "Profile Image is Required."));
      if (duplicate) {
        return res
          .status(409)
      ...
      const result = await PersonalAssistantService.create({ ...req.body, location, profileImageUrl });
      if (!result) {
        return res
          .status(400)
  ...
  static async updatePersonalAssistantById(
    ...
    try {
      ...
      if (!record) {
        return res
          .status(404)
          .json(new ApiError(404, "Assistant not found."));
      }

      let imageUrl;
      if (req?.body?.image && record.profileImageUrl)
        imageUrl = await extractImageUrl(req?.body?.image, record.profileImageUrl as string);

      const result = await PersonalAssistantService.updateById(id, { ...req.body, location, profileImageUrl: imageUrl || profileImageUrl });
      if (!result) {
        return res
          .status(400)
