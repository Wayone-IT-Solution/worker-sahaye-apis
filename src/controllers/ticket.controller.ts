...
export const addInteraction = async (
  ...
  try {
    ...
    const userExist = await Passenger.findById({
      _id: role === "passenger" ? initiator : receiver,
    });
    if (!userExist)
      return res.status(404).json(new ApiError(404, "User not found"));

    const agentExist = await Agent.findById({
      _id: role === "passenger" ? receiver : initiator,
    });
    if (!agentExist)
      return res.status(404).json(new ApiError(404, "Agent not found"));

    const interaction = createInteractionObject({
      action, // for passenger interaction it must be "commented" and for agent can be anything
      content, // shows the interaction content
      receiver, // if role is passenger then it will be agent ID or vice-versa
      initiator, // if role is passenger then it will be passenger ID or vice-versa
      receiverType: role === "passenger" ? "Agent" : "Passenger",
      initiatorType: role === "passenger" ? "Passenger" : "Agent",
    });
...
const getData = async (id: any, role: any): Promise<any> => {
  ...
  if (ticketData?.interactions?.length > 0) {
    ticketData.interactions.forEach((action: any) => {
      const isUser = role === "passenger";
      const isSender =
        (isUser && action?.initiatorType === "Passenger") ||
