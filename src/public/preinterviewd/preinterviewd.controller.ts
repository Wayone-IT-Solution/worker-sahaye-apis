...
import { PreInterviewed, VerificationStatus } from './../../modals/preinterviewd.model';
...
export class PreInterviewedController {
  static async createPreInterviewed(
    ...
    try {
      ...
      const result = await PreInterviewedService.create({ user, document, remarks, interviewedAt });
      if (!result) {
        return res
          .status(500)
          .json(new ApiError(500, "Something went wrong while creating Pre Interviewed."));
      }
      return res
        .status(201)
        .json(new ApiResponse(201, result, "Pre Interviewed submitted successfully."));
    } catch (err) {
      next(err);
    }
  }
  ...
  static async updatePreInterviewedById(
    ...
    try {
      ...
      if (!mongoose.Types.ObjectId.isValid(verificationId))
        return res.status(400).json(new ApiError(400, "Invalid Pre Interviewed doc ID"));

      const existingVerificationRecord: any = await PreInterviewedService.getById(
        verificationId
      );
      if (!existingVerificationRecord)
        return res.status(404).json(new ApiError(404, "Pre Interviewed Doc not found"));

      if (existingVerificationRecord?.status === VerificationStatus.APPROVED)
        return res.status(404).json(new ApiError(404, "Pre Interviewed already approved"));

      const { status, remarks, document, interviewedAt } = req.body;
      const normalizedData = {
        remarks: remarks?.trim() || existingVerificationRecord.remarks || "",
        interviewedAt: interviewedAt || existingVerificationRecord.interviewedAt || "",
    ...
    try {
      ...
      if (existingVerificationRecord?.status === VerificationStatus.APPROVED) {
        return res.status(400).json(
          new ApiError(400, "Pre Interviewed is already approved and cannot be deleted or modified.")
        );
      }
